...
class RunModel:
    ...
    def train_helper(self, model, keypoints_loader, num_iteration):
        ...
        for idx in range(1, num_iteration + 1):
            ...
            source_ten = torch.as_tensor(iterator_data[0], dtype=torch.float).view(-1, 1)[:20] # TODO: remove crop (20 for testing)
    ...
    def evaluate_model_own(self, text2kp):
        ...
        with torch.no_grad():
            in_ten = torch.as_tensor(nexty[0], dtype=torch.float).view(-1, 1)[:20]  # crop to 20 for testing
...
if __name__ == '__main__':
    ...
    source_dim = 20  # length of source vocab dictionary (current trash data = 5) TODO: get automatically
